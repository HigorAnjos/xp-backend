...
describe('Controller Balance deposit', function () {
  it('Deve retornar corretamente status code, e objeto com o saldo atualizado', async function () {
    ...
    sinon.stub(service, 'deposit').resolves(MocBalance);

    await controller.deposit(req, res);
    expect(res.status.calledWith(201)).to.be.true;
    expect(res.json.calledWith({ balance: MocBalance })).to.be.true;
    expect(service.deposit.calledWith(req.client.id, req.body.balance)).to.be.true;
